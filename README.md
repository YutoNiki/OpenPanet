...
ツールバー右端のボタンでオン／オフ（**起動時はオフ**）。オンにすると、ペンを離した瞬間に判定し、直線・楕円（円）・3〜6角形に置き換えます。

判定の流れ:

1. 対角長が 56px 未満、または線長が対角長の 0.8 倍未満のストロークは対象外（手書き文字を巻き込まないため）
2. 始点と終点が離れていれば開いた線 → RDP で 2 点に落ちれば直線
3. 閉じていれば、多角形と楕円それぞれの当てはめ誤差を比較
   - 多角形: RDP の粒度を 6 通り変えて頂点を求め、誤差が対角長の 2% 未満のうち最も頂点が少ないものを採用
   - 楕円: 外接矩形から求めた楕円との半径方向の誤差が対角長の 4.5% 未満なら採用
4. 頂点間の距離と辺の長さのばらつきが小さければ正多角形に整える。向き（回転角）は描いたときのものを保持

合成データでの検証結果は 28/28。ノイズ量 ±4〜12px の円・楕円・三角形・四角形・五角形・六角形・長方形・直線をすべて正しく分類し、なぐり書き・渦巻き・S 字・小さな丸文字は補正しませんでした。
